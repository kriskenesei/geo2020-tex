...
\begin{figure}
    ...
    \caption[Illustration of my top-level methodology]{Flowchart-style illustration of the top-level methodology of my dissertation research.}
...
\begin{figure}[h]
    ...
    \caption[Illustration of the top-level pipeline steps]{Illustration of the top-level steps and output of my processing pipeline.}
    \label{fig:workflow}
\end{figure}
...
\begin{figure}
    ...
    \caption[Flowchart illustrating of the NBRS generation step]{Flowchart-style illustration of the \ac{nbrs} generation step of the pipeline.}
...
\begin{figure}
    ...
    \caption[Flowchart illustrating of the elevation estimation step]{Flowchart-style illustration of the elevation estimation step of the pipeline.}
...
\begin{figure}
    ...
    \caption[Flowchart illustrating of the Lidar segmentation step]{Flowchart-style illustration of the Lidar segmentation step of the pipeline.}
...
\begin{figure}
    ...
    \caption[Illustration of the Lidar segmentation step]{This figure illustrates the plane-fitting step of the Lidar segmentation procedure. Each of the vertices of the preliminary 3D-NWB geometry is fitted with a plane based on the nearby Lidar points. In the case of well-exposed roads, this results in planes lying on the respective road surfaces, labelled "normal plane fits". Where complete occlusion occurs, the plane will be shifted to the elevation of the occluding object, which is a bridge in this illustration - hence the plane fit is labelled "plane fit on bridge". In the case of partial occlusion, the plane will be shifted moderately relative to the true road surface, and potentially tilted. In this illustration, and example is shown where this occurs at the edge of the bridge, labelled "transitional plane fit".}
...
\begin{figure}
    ...
    \caption[Illustration of the edge approximation step]{The edge approximation procedure is illustrated in this figure. The cross-sections are either accepted (black line segments) or skipped if they fail the threshold-based evaluations (red dashed line segments). They may fail to generate in the first place, if there is insufficient \ac{ahn3} and \ac{dtb} data. The figure also shows an example of the cross-sections and the preliminary edges being shifted partially off-the road due to the incorrect georeferencing of NWB locally. The inset illustrates the details of the edge point identification sub-process. The third inlier point on both sides of \ac{nwb} relative to the line fit is taken to represent a suitable edge point.}
...
\begin{figure}
    ...
    \caption[Flowchart illustrating of the active contour optimisation step]{Flowchart-style illustration of the active contour optimisation step of the pipeline.}
...
\begin{figure}
    ...
    \caption[Illustration of the TIN construction step]{This figure illustrates the TIN construction step visually. In the initialisation step, the seed line (green line) is constructed from the road edges, and it is used to insert seed points (green points) into the \ac{tin}. Further points are then inserted conditionally, and they may either refine (red points) or grow (yellow points) the triangulation. In the extension step, the algorithm progresses away from the seed line to further consider points for insertion. For instance, in the 3\textsuperscript{rd} extension, the points between the lines labelled "extension 3" and "extension 4" on both sides on the seed line would be considered.}
...
\begin{figure}
    ...
    \caption[Flowchart illustrating the details of TIN initialisation and extension]{Flowchart-style illustration of the details of \ac{tin} initialisation and extension.}
...
As implementing such a complex algorithm in this pipeline step was not originally intended, the biggest challenge here was to come up with a good solution within the timeframe reserved for the project. In terms of the methods themselves, I spent the most amount of effort on developing a solution that allows the road surface to grow into regions not covered by the preliminary or optimised edges. While in all cases, the surface generated by \ac{tin} initialisation is adequate to generate 3D-NWB, my desire was to not only model the central, traffic-occupied part of the road, but to grow it to the real-life edges of the paved surface as best as possible.

I initially wished to build the \ac{tin} as a single growing operation, but this proved to be prone to spreading into off-road regions. Depending on what order certain groups of points are examined in, even relatively sharp breaks in the underlying surface may appear smooth. A straightforward way to deal with this problem would have been to decrease the query radius used in the buffer queries, but this turned out to prevent the \ac{tin} from growing at all, simply because this results in too large triangles being generated. This is the reason why I eventually implemented the final version of the method as a combination of first initialising a conservative \ac{tin} in a large-scale growing operation, and then extending it carefully by examining thin "layers" of additional points progressing outwards from the centre of the road.

Performing growing-type insertions was also a challenge. Simply interpolating in triangles is not a possibility here, but the importance of assessing the compliance of the tested points with the neighbourhood is ever so important - in fact, it largely depends on these surface growing insertions, whether a \ac{tin} might accidentally spread into an off-road area, or not. The careful treatment of these insertions via carefully detecting where they occur, and by using additional triangles rather than just one for the conditional insertion, turned out to be a key step to achieve my desired results.
...
\begin{figure}
    ...
    \caption[Flowchart illustrating the elevation interpolation step]{Flowchart-style illustration of the elevation interpolation step of the pipeline.}
