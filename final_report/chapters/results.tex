...
The effects of the refinement step can be observed in the shape of the \textit{lower} set of 3D centrelines shown on the left in Figure \ref{fig:elevationestimation2}. The road passing over the other causes occlusion, meaning that Lidar data is unavailable for the surface of the lower road for a short distance. Before the refinement step, the centrelines of the lower set of roads (motorway lanes) thus became snapped to the elevation of the one above. The resulting outliers were then easily identified and eliminated by the refinement step, and new values were interpolated based on the polynomial model that was fitted on the NBRS.
